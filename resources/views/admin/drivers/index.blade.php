    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center">
                ...
                {{ __('Quản lý Danh sách Tài Xế') }}
            </h2>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">
                {{ __('Quản lý danh sách tài xế nội bộ để lựa chọn khi thực hiện cho xe đi (Check-out)') }}
            </p>
        </div>
    </div>
        ...
        <div class="lg:col-span-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
            ...
            <div class="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex flex-col sm:flex-row gap-4 items-center justify-between">
                <h3 class="font-bold text-slate-700 dark:text-slate-200 flex items-center shrink-0">
                    <i class="fas fa-list mr-2 text-slate-400"></i> {{ __('Danh sách tài xế') }}
                </h3>
                <form action="{{ route('admin.drivers.index') }}" method="GET" class="w-full sm:max-w-xs flex gap-2">
                    <div class="relative flex-1">
                        <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('Tìm tên, số điện thoại...') }}" 
                    ...
                    @if($search)
                    <a href="{{ route('admin.drivers.index') }}" class="px-3 py-2 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-sm hover:bg-slate-300 transition-colors">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider bg-slate-50 dark:bg-slate-950/30">
                            <th class="py-4 px-6">{{ __('Họ và tên tài xế') }}</th>
                            <th class="py-4 px-6">{{ __('Số điện thoại') }}</th>
                            <th class="py-4 px-6 text-center">{{ __('Ngày thêm') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-150 dark:divide-slate-850">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                            <td class="py-4 px-6 font-semibold text-slate-800 dark:text-slate-100 text-base">
                                {{ $driver->name }}
                            </td>
                            <td class="py-4 px-6 text-slate-600 dark:text-slate-400 text-sm">
                                {{ $driver->phone ?: __('Chưa cập nhật') }}
                            </td>
                            ...
                            <td class="py-4 px-6 text-center space-x-1.5 whitespace-nowrap">
                                <button type="button" 
                                        onclick="openEditModal({{ $driver->id }}, '{{ addslashes($driver->name) }}', '{{ addslashes($driver->phone) }}')"
                                ...
                                <form action="{{ route('admin.drivers.destroy', $driver->id) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Bạn có chắc chắn muốn xóa tài xế này khỏi hệ thống không?') }}')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-rose-500 hover:text-rose-600 dark:hover:text-rose-455 p-1.5 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors" title="{{ __('Xóa') }}">
                        ...
                        <tr>
                            <td colspan="4" class="py-12 text-center text-slate-500">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-user-tie text-4xl mb-3 opacity-40"></i>
                                    <p class="text-sm font-medium">{{ __('Không tìm thấy thông tin tài xế nào.') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6 space-y-5 transition-colors">
            <div class="border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 class="font-bold text-slate-800 dark:text-slate-150 text-base uppercase tracking-wider flex items-center">
                    <i class="fas fa-plus-circle text-blue-500 mr-2"></i> {{ __('Thêm tài xế mới') }}
                </h3>
            </div>
            ...
            <form action="{{ route('admin.drivers.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5">{{ __('Họ và tên tài xế') }} <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" required placeholder="{{ __('Ví dụ: Nguyễn Văn A') }}" 
                ...
                <div>
                    ...
                    <input type="tel" name="phone" placeholder="{{ __('Ví dụ: 0987654321') }}" 
                           class="w-full border border-slate-300 dark:border-slate-850 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 rounded-lg p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
...
<div id="edit-modal" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl z-50 hidden transition-all duration-300 opacity-0 scale-95 border border-slate-200 dark:border-slate-800 overflow-hidden">
    ...
    <div class="bg-blue-600 px-5 py-4 flex justify-between items-center text-white">
        <h3 class="font-bold flex items-center"><i class="fas fa-edit mr-2"></i> {{ __('Chỉnh sửa tài xế') }}</h3>
    ...
    <!-- Form -->
    <form id="edit-driver-form" method="POST" class="p-6 space-y-4">
        @csrf
        @method('PUT')
...
<script>
    ...
    function openEditModal(id, name, phone) {
        editForm.action = `/admin/trucks/drivers/${id}`;
